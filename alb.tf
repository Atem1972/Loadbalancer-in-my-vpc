...
resource "aws_lb_target_group_attachment" "attach-app1" {
  ...
  target_id        = aws_instance.server1.id  # recalling id from ec2 instance
  port             = 80
}
...
resource "aws_lb_target_group_attachment" "attach-app2" {
  target_group_arn = aws_lb_target_group.alb-target-group.arn # recalling id from target group
  target_id        = aws_instance.server2.id  # recalling id from ec2 instance
  port             = 80
}
